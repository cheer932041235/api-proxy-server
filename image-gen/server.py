...
BACKENDS = [
    {"name": "Pool-A", "url": os.environ.get("CHATGPT2API_URL", "http://host.docker.internal:3002"), "key": "sk-gpt2api-secret"},
    {"name": "Pool-B", "url": os.environ.get("CHATGPT2API_URL", "http://host.docker.internal:3002"), "key": "sk-gpt2api-secret"},
]
...
- Do NOT include any explanation or metadata"""
    ...
    def _handle_generate(self):
        ...
        if not results:
            self._json_response(500, {"error": f"All pools failed: {'; '.join(errors)}"})
            return
