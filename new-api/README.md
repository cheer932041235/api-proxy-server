...
| 项目 | 值 |
|------|---|
| Web UI | http://170.106.65.175:3001 |
| 账号 | `root` |
| 密码 | `Proxy2026!` |
| API 端点 | http://170.106.65.175:3001/v1/chat/completions |
...
- **自用模式**: 已开启（无需配置模型定价）
- **渠道**: chatgpt2api（代理 `http://172.17.0.1:3002`，密钥 `sk-gpt2api-secret`）
...
用户端 → New-API:3001 → chatgpt2api:3002 → ChatGPT Plus（文本 + 出图）
                      → AIClient-2-API:3000 → Gemini/Kiro/Codex（免费 OAuth）
```
